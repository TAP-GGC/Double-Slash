...
func tip():
	var message = ""
	if (enemyData.type == "Int"):
		message = "Remember ints are whole numbers like 10, 5, 2, 1, etc."
	elif(enemyData.type == "Double"):
		message = "Remember Double are numbers with a decimal like 2.2, 1.5, 0.0, etc."
	elif(enemyData.type == "String"):
		message = "Remember Strings are characters in qutations like \"HelloWorld\" "
	elif(enemyData.type == "If-Statement"):
		message = "Remember If-Statments are intialized like this if(correctAnswer)"
	elif(enemyData.type == "Loop"):
		message = "Remember Loops are constructed like this for(int i = 0; i < students.size();i++)"
	elif(enemyData.type == "Boolean"):
		message = "Remember booleans are true or false statements"
...
func enemy_death_handler():
	...
	if (current_enemy_health == 0 and enemyData.name == "VByte"):
		...
		emit_signal("enemy_defeated")
		get_tree().paused = false
		get_tree().change_scene_to_file("res://Scenes/EnviromentScenes/Ending.tscn")
